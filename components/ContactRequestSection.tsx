...
export default function ContactRequestSection({
  ...
  const formSubheading = selectedService
    ? `Share the essentials now. Details such as dimensions, wallpaper status, timing, and photos can be added later for ${selectedService.title.toLowerCase()}.`
    : subheading;
    ...
    <AnimatedSection>
      <section className="py-14 md:py-16">
        <div className="mx-auto grid max-w-7xl gap-8 px-4 sm:px-6 lg:grid-cols-[1.08fr_0.92fr] lg:px-8">
          <div className="min-w-0 border border-gray-200 border-t-accent bg-white p-6 md:p-8 lg:p-10">
          ...
          <aside className="min-w-0 space-y-5">
            ...
            <MobileCarousel label="Direct Contact">
              {contactMethods.map((method, index) => {
                ...
                const content = (
                  <>
                    <span className="flex h-10 w-10 items-center justify-center border border-accent/25 text-accent">
                ...
                return method.href ? (
                  <a
                    key={method.label}
                    href={method.href}
                    className="flex min-h-28 items-center gap-4 border border-gray-200 border-t-accent bg-white p-5"
                  >
                    {content}
                  </a>
                ) : (
            ...
            <div className="hidden border border-gray-200 bg-white md:grid">
              {contactMethods.map((method, index) => {
                const Icon = methodIcons[index] ?? Mail;
                const content = (
                  <>
